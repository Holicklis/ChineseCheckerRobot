...
if __name__ == "__main__":
    ...
    while not board.has_game_ended():
        ...
        path = player.get_move(board)  # e.g. from minimax or from the user

        # Instead of calling board.apply_path(path) all at once,
        # do each step individually so you can sleep & display
        for i in range(len(path) - 1):
            origin = path[i]
            destination = path[i+1]

            # Move single step
            board.move_piece_to_tile(origin, destination)

            # Print board after each step
            board.print_board()

            # Sleep for 1 second (or however long you like)
            time.sleep(1)
